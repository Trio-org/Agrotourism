...
const TourCard = ({ tour }) => {
  return (
    <Link to={ `/details/:${tour.id}`} className='max-w-lg sm:max-w-md  rounded-lg shadow hover:shadow-lg  hover:scale-105 transition-all duration-300'>
      ...
      <div className="h-1/2 px-3 py-2">
        ...
        <div className="flex justify-between items-center">
          ...
          <Link to={'/details/:id'} className='bg-[#009a74] bg-opacity-90 hover:bg-opacity-100 text-white text-center font-semibold px-4 py-2 rounded-md'>Book</Link>
        </div>
      </div>
    </Link>
    // <Link to={`/details/${tour.id}`} className="relative hover:-translate-y-10 cursor-pointer bg-white  transition-all duration-300 flex w-full max-w-sm sm:max-w-xs flex-col overflow-hidden rounded-lg border border-gray-100 hover:shadow-2xl shadow-md">
    //   <a
